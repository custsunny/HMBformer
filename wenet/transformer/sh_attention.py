...
from wenet.branchformer.pooling import FastSelfAttention
...
class sh_attention(nn.Module):
    ...
    >>> net = sh_attention(512, 2048, num_heads=8)
    ...
    def __init__(
        ...
        self.pooling = FastSelfAttention(
            input_output_dim,
            num_heads,
            dropout_rate,
        )
        ...
        self.merge_proj = torch.nn.Linear(input_output_dim + input_output_dim, input_output_dim)
    ...
    def forward(
        ...
        out, dummy_att_weights = self._forward_mixing(query, key_padding_mask)
        pooling = self.pooling(out, key_padding_mask)
        x_concat = torch.cat([out, pooling], dim=-1)
        x_concat = self.merge_proj(x_concat)
        x_concat_out, dummy_att_weights = self._forward_mixing(x_concat, key_padding_mask)

        return x_concat_out, dummy_att_weights
        ####################################################################################
        # # transpose back
        # out = out.transpose(1, 2)
        # out = self.layer_norm(out)
        # key_padding_mask = key_padding_mask.unsqueeze(1)
        # pooling = self.pooling(query, key_padding_mask)
        # pooling = self.layer_norm(pooling)
        # x_concat = torch.cat([out, pooling], dim=-1)
        # dummy_att_weights = torch.zeros(
        #     (bsize, seq_len, seq_len), device=out.device
        # )
        # return x_concat, dummy_att_weights
        ####################################################################################
        # # transpose back
        # out = out.transpose(1, 2)
        # #out = self.layer_norm(out)
        ...
        #
        # x = self.summary(x, attention_mask=key_padding_mask)
        # x_concat = (
        #     out * x
        # )  # (batch, time, size)
        # dummy_att_weights = torch.zeros(

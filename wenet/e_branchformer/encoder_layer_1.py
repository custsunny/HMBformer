...
from wenet.Interformer.convolution_extraction import ConvolutionExtraction
...
class EBranchformerEncoderLayer(torch.nn.Module):
    ...
    def forward(
        ...
        x_att, new_att_cache = self.attn(x1, x1, x1, mask, mask_pad, pos_emb)

        # x_att = self.merge_proj(x_att)
        ...
        x2, x_cgmlp, new_cnn_cache = self.cgmlp(x2, x_att, mask_pad, cnn_cache)
        # x2 = self.dropout(x2)

        # Merge two branches

        # fusion block
        x1 = x_att + self.conv_extract(x_att, x_cgmlp)
        x2 = x_cgmlp + x2
        x1 = self.dropout(x1)
        x2 = self.dropout(x2)

    ...
    def train(self, model, optimizer, scheduler, train_data_loader,
        ...
        with model_context():
            for batch_idx, batch_dict in enumerate(train_data_loader):
                ...
                # 记录语音长度和显存占用
                info_dict['audio_lengths'] = (batch_dict['pcm_length'].float() / 16000).mean().item() # 例如：音频长度
                info_dict['train_time'] = time.time() - batch_start_time  # 训练时间
                info_dict['memory_usage'] = torch.cuda.max_memory_allocated() / (1024 ** 3)  # 显存占用 GB
                ...
                if (self.step +
                    ...
                    loss_dict = self.cv(model, cv_data_loader, writer, configs)
    ...
    def cv(self, model, cv_data_loader, writer, configs):
        ...
        with torch.no_grad():
            for batch_idx, batch_dict in enumerate(cv_data_loader):
                ...
                info_dict['audio_lengths'] = (batch_dict['pcm_length'].float() / 16000).mean().item() # 例如：音频长度
                info_dict['cv_time'] = time.time() - cv_start_time
                # write cv: log
                log_per_step(writer,
                             info_dict=info_dict,

...
def log_per_step(writer, info_dict, timer: Optional[StepTimer] = None):
    ...
    # 新增记录的参数
    audio_lengths = info_dict.get("audio_lengths", 0)
    train_time = info_dict.get("train_time", 0)
    memory_usage = info_dict.get("memory_usage", 0)

    # CV 专用参数
    audio_dev_lengths = info_dict.get("audio_dev_lengths", 0)
    cv_time = info_dict.get("cv_time", 0)
    ...
    if tag == "TRAIN" and rank == 0 and writer is not None:
        if (train_engine == "deepspeed" and is_gradient_accumulation_boundary
            ...
            # 记录新增的参数
            writer.add_scalar('train/audio_lengths', audio_lengths, step)
            writer.add_scalar('train/train_time', train_time, step)
            writer.add_scalar('train/memory_usage', memory_usage, step)


    # CV Tensorboard
    elif tag == "CV" in tag and rank == 0 and writer is not None:
        for name, value in loss_dict.items():
            writer.add_scalar('cv/{}'.format(name), tensor_to_scalar(value),
                              step)
        writer.add_scalar('cv/cv_time', cv_time, step)
        writer.add_scalar('cv/audio_dev_lengths', audio_dev_lengths, step)
        logging.info(
